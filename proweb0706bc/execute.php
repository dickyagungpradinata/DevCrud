...
if ($proc == "usrdel") {
    ...
    //end for
    $query = "INSERT INTO users (username,password,fullname,email,telp,banned,akses) "
            . "VALUE ('$username',PASSWORD('$password'),"
            . "'$fullname','$email','$telp','$baned',$aks)";    
}else if ($proc == "usredt" && $usrid > 0) {
    $query = "SELECT id,username,fullname,email,telp,banned,akses FROM users WHERE id=".$usrid;
